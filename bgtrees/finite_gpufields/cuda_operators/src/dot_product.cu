...
namespace myfunctor {
    ...
    template <typename T, bool singleBatch>
        __global__ void DotProductCudaKernel(const int bs, const int o1, const int o2, const int size_i, const int p, const T* x, const T* y, T* out) {
            /*
    ...
    template <typename T, bool singleBatch>
        struct DotProductFunctor<GPUDevice, T, singleBatch> {
            void operator()(const GPUDevice& d, const int bs, const int o1, const int o2, const int size_i, const int p, const T* x, const T* y, T* out) {
                ...
                */ 
                ...
                DotProductCudaKernel<T, singleBatch>
                    <<<block_count, thread_per_block, 0, d.stream()>>>(bs, o1, o2, size_i, p, x, y, out);
            }
        };
